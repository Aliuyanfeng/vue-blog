  ...
  <div id="container">
    <el-form ref="form" :model="form">
      ...
      <el-form-item label="发布内容">
        <mavon-editor @change="handle" ref="md" @imgAdd="$imgAdd" @imgDel="$imgDel"></mavon-editor>
      </el-form-item>
      <el-form-item>
  ...
  methods: {
    ...
    async submit() {
      ...
      if (
        ...
        this.form.classify.trim().length <= 0
      )
        return this.open3();
      const { data } = await this.$http.post("/api/write", this.form);
      if (data.status === 0) {
        this.open2();
        // 发布文章之后 重定向到文章列表
        this.$router.push({
